...
def save_file(file, folder):
    ...
    return file_path
url = "https://apis-public.congtrinhviettel.com.vn/key_llm" 
...
def handle_request(
    ...
    if Image:
        ...
        with open(image_path, "rb") as f_image:
            ...
            if response['status'] == 200 and len(response) > 1:
                ...
                for ob in objects:
                    query_text += ob + ','
                # chat_out = predict_rasa_llm_for_image(query_text, IdRequest, NameBot, User)
                results.update({
    ...
    if Voice:
        ...
        print("check response in main run", response)
        chat_out = response
        results.update({
            "content": chat_out['out_text'],
            "terms": chat_out['terms'],
